...
class TestGenerator:
    ...
    def generate_verbal_number(self):
        ...
        elif style < 0.6: # "целых"
            inp = f"{int_word} целых {frac_words}"
        else: # "целых ... десятых/сотых"
    ...
    def get_simple_var(self):
        ...
        # Число + переменная (через пробел в input -> * в expected)
        if roll < 0.6:
            num = self.generate_verbal_number()
            return {"input": f"{num['input']} {name}", "expected": f"{num['expected']} * {name}"}

        # Две переменные (через пробел в input -> * в expected)
        if roll < 0.8:
    ...
    def fill_template(self, tmpl, depth, simple):
        ...
        if "special_cases" in tmpl and tmpl["special_cases"] and random.random() < 0.5:
            ...
            if sym == "^": exp = f"({arg['expected']})^({val_const})"
            elif sym == "root": exp = f"root({val_const}, {arg['expected']})"
            else: exp = f"{sym}({arg['expected']})"
        ...
        if sym == "frac":
            a = args_expected[0] if len(args_expected) > 0 else "a"
            b = args_expected[1] if len(args_expected) > 1 else "b"
            exp = f"frac({a}, {b})"
        elif sym == "/":
            a = args_expected[0] if len(args_expected) > 0 else "a"
            b = args_expected[1] if len(args_expected) > 1 else "b"
    ...
    def generate_target(self, term_sym, simple=False):
        ...
        if not tmpl: raise ValueError(f"Term {term_sym} not found")
        ...
        if not simple and random.random() > 0.3:
            ...
            if random.random() > 0.5:
                return {"input": f"{expr['input']}{op['in']}{noise['input']}",
                        "expected": f"{expr['expected']}{op['exp']}{noise['expected']}"}
            else:
                return {"input": f"{noise['input']}{op['in']}{expr['input']}",
